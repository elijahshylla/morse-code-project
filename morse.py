...
class Visualizer:
    ...
    def __init__(self):
        ...
        self.coded_textbox.tag_config("highlight",
            background="#00c2a8",
            foreground="#001a17")
    ...
    def import_audio(self):
        ...
        AUDIO_DIR = BASE_DIR / "morse_audio"

        self.dot_5 = AUDIO_DIR / "dot_5.wav"
        self.dash_5 = AUDIO_DIR / "dash_5.wav"
        self.dot_10 = AUDIO_DIR / "dot_10.wav"
        self.dash_10 = AUDIO_DIR / "dash_10.wav"
        self.dot_20 = AUDIO_DIR / "dot_20.wav"
        self.dash_20 = AUDIO_DIR / "dash_20.wav"
        ...
        self.audios = {
            "5d": self.dot_5_audio,
            "10d": self.dot_10_audio,
            "20d": self.dot_20_audio,
            "5D": self.dash_5_audio,
            "10D": self.dash_10_audio,
            "20D": self.dash_20_audio
        }
    ...
    def simulate(self):
        ...
        for symbol in morse_code:
            if symbol == '.':
                self.timeline.append((True, self.DOT, wpm+'d', char_index))
                self.timeline.append((False, self.SYMBOL_GAP, None, None))
            elif symbol == '-':
                self.timeline.append((True, self.DASH, wpm+'D', char_index))
                self.timeline.append((False, self.SYMBOL_GAP, None, None))
            elif symbol == ' ':
                self.timeline.append((False, self.LETTER_GAP, None, None))
            elif symbol == '/':
                self.timeline.append((False, self.WORD_GAP, None, None))
    ...
    def play(self):
        ...
        if self.stop:
            ...
            self.playing = False
            return
        state, duration, sound, char_index = self.timeline[self.index]
        self.highlight_symbol(char_index)
    ...
    def highlight_symbol(self, index):
        self.coded_textbox.tag_remove("highlight", "1.0", END)

        if index is not None:
            start = f"1.0+{index}c"
            end = f"{start}+1c"
            self.coded_textbox.tag_add("highlight", start, end)
            self.coded_textbox.see(start)
...
if args.write and output_string:
    ...
    print(output_string)from pathlib import Path
from tkinter import Tk
from tkinter import ttk
import os
os.environ['PYGAME_HIDE_SUPPORT_PROMPT'] = "hide"
